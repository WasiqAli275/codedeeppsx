...
<style>
    ...
    .cloud-header {
        ...
        /* run gradient + fade only once and keep final state */
        animation: 
    ...
    .cloud-card {
        ...
        /* run entrance + float only once, keep end state */
    ...
    .cloud-card:hover {
        ...
        /* Pause hover animation behaviour unchanged */
...
def main():
    ...
    if selected_ts:
        ...
        # Welcome screen rendered as full HTML (ensures CSS + animations render correctly)
        html = """
        <style>
        ...
        .cloud-card{background:#fff;padding:20px;border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,0.08);max-width:1000px;margin:10px auto;font-family:inherit;
          /* run entrance + float once and hold final state */
          animation: fadeInUp .6s ease-out 0s 1 forwards, cloudFloat 6s ease-in-out 0s 1 forwards;
          animation-fill-mode: both;
        }
        .cloud-header{font-size:2.25rem;font-weight:700;text-align:center;margin-bottom:8px;background:linear-gradient(45deg,#1e90ff,#4169e1,#87ceeb);background-size:300% 300%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;
          /* run gradient + fade once */
          animation: gradientFlow 5s ease 0s 1 forwards, fadeInUp 1s ease-out 0s 1 forwards;
          animation-fill-mode: both;
        }
        .cloud-subheader{font-size:1.1rem;color:#555;text-align:center;margin-bottom:12px;opacity:.95; /* run once */ animation:fadeInUp 1.1s ease-out 0s 1 forwards; animation-fill-mode: both;}
